...
function AuthProvider({ children }: AuthProviderData) {
  ...
  async function signIn() {
    try {
      ...
      if (results.type === 'success' && results.params.error !== 'access_denied') {
        ...
        setUserToken(results.params.access_token);
      }
    } catch (error) {
      // throw an error
  ...
  return (
    <AuthContext.Provider value={{ user, isLoggingOut, isLoggingIn, signIn, signOut }}>{children}</AuthContext.Provider>
  );
}
